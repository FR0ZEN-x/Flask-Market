from market import app, db # Importing the app & db variable from the market package directly because they are defined in __init__.py
from flask import render_template, redirect, url_for, flash, request
from market.models import Item, User
from market.forms import RegisterForm, LoginForm, PurchaseForm, SellForm, AddItems
...
def market_page():
    ...
    if request.method == "GET":
        owned_items = Item.query.filter_by(owner=current_user.id)  # Return a list of all the rows in the table(Item) from the database(market.db) where the owner is the current user.
        items = Item.query.filter_by(owner=None) # Return a list of all the rows in the table(Item) from the database(market.db) where the owner is None.
...
def admin_page():
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash(f'The item {item_to_create.name} has been added to the market!', category='success')
